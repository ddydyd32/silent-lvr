#!/bin/bash -l
#SBATCH --job-name=monet-rl-thymerl-necessity0-attn1-latent0-includelatentsample1-kl0.01-temp0.5-rollout8-numgpu4
#SBATCH --output=monet-rl-thymerl-necessity0-attn1-latent0-includelatentsample1-kl0.01-temp0.5-rollout8-numgpu4.-%j.log
...
export a_very_big_data_disk=xxxxxxxxxxxxxxxxxxxxxxxxxxxx
export home=xxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
export RAY_OBJECT_STORE_MEMORY=$((8 * 1024 * 1024 * 1024)) # 8GB
...
MODEL_PATH=${a_very_big_data_disk}/Monet-SFT-7B/stage3
latent_size=10
...
export LATENT_NECESSITY_REPLAY_DEBUG=0
export LATENT_NECESSITY_REWARD_N=0.0
export add_attn_to_reward=0
...
experiment_name=monet-rl-${datasetname}-necessity${LATENT_NECESSITY_REWARD_N}-attn${add_attn_to_reward}-latent${add_latent_to_reward}-includelatentsample${include_latent_sample}-kl${KL_COEF}-temp${TEMPERATURE}-rolloutn${ROLLOUT_N}-numgpu${N_GPUS_PER_NODE}
