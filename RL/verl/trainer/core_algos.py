...
@torch.no_grad()
def compute_policy_loss_per_sample(
    old_log_probs: torch.Tensor,
    log_probs: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """Compute PPO policy loss per sample (mean over valid response tokens)."""
    negative_approx_kl = torch.clamp(log_probs - old_log_probs, min=-10.0, max=10.0)
    ratio = torch.exp(negative_approx_kl)
    clipped_ratio = torch.exp(
        torch.clamp(negative_approx_kl, np.log(1.0 - clip_ratio_low), np.log(1.0 + clip_ratio_high))
    ...
    return VF.masked_mean(final_pg_loss, response_mask, dim=-1)
