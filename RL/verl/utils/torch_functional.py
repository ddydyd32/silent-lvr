...
def get_attention_mask_from_padded_input_ids(
    input_ids: torch.Tensor, pad_token_id: int = 0, dtype: torch.dtype = torch.long
) -> torch.Tensor:
    """Get the mask for the left-padded 2D tensor input ids, the mask will be 1 for non-pad tokens."""
    input_mask = input_ids.ne(pad_token_id).long()
    return input_mask.to(dtype)


def pad_2d_list_to_length(
    response: List[List[int]], pad_token_id: int, max_length: Optional[int] = None, left_or_right: str = "right"
) -> torch.Tensor:
    ...
    for sub_list in response:
        if left_or_right == "right":
            if len(sub_list) > target_length:
                ...
                result_responses.append(tuple(sub_list) + (pad_token_id,) * (target_length - len(sub_list)))
        elif left_or_right == "left":
            if len(sub_list) > target_length:
                result_responses.append(tuple(sub_list[:target_length]))
            else:
                result_responses.append((pad_token_id,) * (target_length - len(sub_list)) + tuple(sub_list))
    tensor = torch.tensor(result_responses)
    return tensor
