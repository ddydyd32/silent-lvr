...
print(f"[Monet/RL/monet_models/transformers/monet_modeling_qwen2_5_vl.py] imported.")
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if output_attentions:# and latent_poss is not None:
            print(f"[Monet/RL/monet_models/transformers/monet_modeling_qwen2_5_vl.py/Qwen2_5_VLFlashAttention2] output_attentions is True")
            # full = torch.arange(query_states.shape[2], device=query_states.device)
            # mask = torch.ones(full.shape[0], dtype=torch.bool, device=query_states.device)
            # mask[latent_poss] = False
            # text_response_pos = full[mask]# & (full >= latent_poss.min())]
            with torch.no_grad():
                q = query_states#[:, :, text_response_pos, :]
                ...
                # upcast attention to fp32
                attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
                # attn_weights = attn_weights[..., latent_poss]
        else:
            attn_weights = None
...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if False and output_attentions:
        ...
        if output_attentions:# and latent_poss is not None:
            ...
            with torch.no_grad():
                ...
                if attention_mask is not None:  # no matter the length, we just slice it
                    causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
                    attn_weights = attn_weights + causal_mask#[:, :, text_response_pos]
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        # latent_poss = kwargs.get('latent_poss', None)
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
            output_attentions=output_attentions and (self.self_attn.layer_idx == 0),
            use_cache=use_cache,
            cache_position=cache_position,
            position_embeddings=position_embeddings,
            # **{'latent_poss': latent_poss} if output_attentions and latent_poss is not None else {}
        )
        ...
        for i,decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    position_ids,
                    past_key_values,
                    output_attentions and i == 0,
                    use_cache,
                    cache_position,
                    position_embeddings,
                    # **{'latent_poss': latent_poss} if i == 0 and output_attentions and latent_poss is not None else {}
                )
            else:
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=(output_attentions and i == 0),
                    use_cache=use_cache,
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
                    # **{'latent_poss': latent_poss} if i == 0 and output_attentions and latent_poss is not None else {}
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # if empty latent_poss for all batches, return all 0
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
        ...
        # if output_attentions:
            # if latent_poss is None or latent_poss.numel() == 0:
                # outputs.attentions = (torch.zeros([inputs_embeds.shape[0], 28, 3, 5], device=inputs_embeds.device),)
