...
def merge_dcp_checkpoint(local_dir: str, hf_upload_path: str = None):
    """Merge a DCP sharded checkpoint into a HuggingFace model.

    Works on a single machine without GPU. Initializes a temporary
    single-process group for DCP loading.
    """
    ...
    elif "ForConditionalGeneration" in architectures[0] or 'QwenWithLVR' in architectures[0]:
    ...
    # Get model's state dict as a template for DCP to fill
    state_dict = model.state_dict()
    model_ckpt_dir = os.path.join(local_dir, "model")

    # Initialize a single-process group for DCP (if not already in a distributed context)
    ...
    dcp.load(state_dict, checkpoint_id=model_ckpt_dir)
    ...
    if hf_upload_path:
        upload_model_to_huggingface(hf_path, hf_upload_path)
...
if __name__ == "__main__":
    ...
    if os.path.isdir(os.path.join(local_dir, "model")):
        print(f"Detected DCP sharded checkpoint in {local_dir}")
        merge_dcp_checkpoint(local_dir, args.hf_upload_path)
        exit()

    # ---- Legacy per-rank .pt merge ----
